...
class Renderer {
    ...
    async loadTitleScreenAssets() {
        ...
        const titleAssets = [
            ...
            ['CLASSIC', 'card_CLASSIC.png'],
            ['CUSTOM', 'card_CUSTOM.png'],
            ['FUN', 'card_FUN.png'],
    ...
    drawTitleScreen(data) {
        ...
        const showCustom = data?.showCustom !== false;
        const maxSelectorIndex = showMultiplayer ? (showCustom ? 4 : 3) : (showCustom ? 3 : 2);
        const selectorIndex = Math.max(0, Math.min(maxSelectorIndex, Number(data?.selectorIndex ?? 0)));
        const modeCard = String(data?.modeCard || 'CLASSIC').replace(/\s+/g, '').toUpperCase() === 'CUSTOM' ? 'CUSTOM' : 'CLASSIC';
        const ratingCard = String(data?.ratingCard || 'FUN').replace(/\s+/g, '').toUpperCase();

        this.drawTitleSprite('LOGO', centerColumn - 13, 1);

        let handColumns;
        if (showMultiplayer && showCustom) {
            // Web/private layout: 1P, 2P, New Level, difficulty, Classic/Custom.
            this.drawTitleCard('1PLAYER', centerColumn - 15, 12, '1PLAYER');
            this.drawTitleCard('2PLAYER', centerColumn - 9, 12, '2PLAYER');
            this.drawTitleCard('NEWLEVEL', centerColumn - 3, 12, 'NEW');
            this.drawTitleCard(ratingCard, centerColumn + 3, 12, ratingCard);
            this.drawTitleCard(modeCard, centerColumn + 9, 12, modeCard);
            handColumns = [centerColumn - 11, centerColumn - 5, centerColumn + 1, centerColumn + 7, centerColumn + 13];
        } else if (showMultiplayer) {
            // Public layout for now: hide Custom/PNG from the title screen and
            // keep the old four-card row: 1P, 2P, New Level, difficulty.
            ...
            handColumns = [centerColumn - 7, centerColumn - 1, centerColumn + 5, centerColumn + 11];
        } else if (showCustom) {
            this.drawTitleCard('1PLAYER', centerColumn - 11, 12, '1PLAYER');
            this.drawTitleCard('NEWLEVEL', centerColumn - 5, 12, 'NEW');
            this.drawTitleCard(ratingCard, centerColumn + 1, 12, ratingCard);
            this.drawTitleCard(modeCard, centerColumn + 7, 12, modeCard);
            handColumns = [centerColumn - 7, centerColumn - 1, centerColumn + 5, centerColumn + 11];
        } else {
            ...
            handColumns = [centerColumn - 4, centerColumn + 2, centerColumn + 8];
        }
    ...
    drawCreditsScreen(data) {
        ...
        this.drawCenteredLevelInfoText('JAVASCRIPT PORT BY SQUIDGY', 15, centerColumn);
        this.drawCenteredLevelInfoText('ALL LEVELS INSTALLED', 18, centerColumn);

        this.drawLevelInfoText('RATING', centerColumn - 11, 21);
        this.drawRightAlignedLevelInfoText(rating, centerColumn + 3, 21);
    ...
    drawLevelHud(hudInfo) {
        ...
        // Icon strip / skill inventory cluster. 
