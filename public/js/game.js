class Game {
    constructor(canvas) {
        ...
        // Custom/PNG levels remain supported by the engine/editor, but public
        // builds can hide the CLASSIC/CUSTOM title toggle until it is ready.
        this.customTitleEnabled = buildConfig.showCustomTitle !== false && buildConfig.showCustomLevels !== false;
        ...
        this.POINTER_EDGE_SCROLL_MARGIN_PX = 16;
        ...
        this.levelSelectionMode = 'classic';
        this.customLevelEntries = [];
        this.customLevelListLoaded = false;
    ...
    async loadCustomLevelList({ force = false } = {}) {
        if (this.customLevelListLoaded && !force) return this.customLevelEntries;
        try {
            this.customLevelEntries = await this.levelLoader.listCustomLevels();
        } catch (error) {
            console.warn('Could not load custom level manifest:', error);
            this.customLevelEntries = [];
        }
        this.customLevelListLoaded = true;
        return this.customLevelEntries;
    }

    normaliseRatingPrefix(value) {
        const safe = String(value || 'FUN').toUpperCase().replace(/\s+/g, '');
        return this.ratingOrder.find(rating => rating.prefix === safe)?.prefix || 'FUN';
    }

    getSelectedRatingPrefix() {
        return this.normaliseRatingPrefix(this.getSelectedRating()?.prefix || 'FUN');
    }

    setLevelSelectionMode(mode = 'classic') {
        const allowCustom = this.customTitleEnabled !== false;
        this.levelSelectionMode = allowCustom && mode === 'custom' ? 'custom' : 'classic';
        this.selectedLevelNumber = 1;
    }

    toggleTitleLevelSource() {
        if (this.customTitleEnabled === false) return;
        this.setLevelSelectionMode(this.levelSelectionMode === 'custom' ? 'classic' : 'custom');
        this.audio?.playSfx?.('chime');
    }

    async startLevelSelection(mode = this.levelSelectionMode) {
        const allowCustom = this.customTitleEnabled !== false;
        this.levelSelectionMode = allowCustom && mode === 'custom' ? 'custom' : 'classic';
        if (this.levelSelectionMode === 'custom') {
            await this.loadCustomLevelList({ force: true });
            this.selectedLevelNumber = Math.min(Math.max(1, this.selectedLevelNumber || 1), this.getCustomLevelCount());
        }
    ...
    getTitleRenderData() {
        return {
            ...
            modeCard: this.levelSelectionMode === 'custom' ? 'CUSTOM' : 'CLASSIC',
            ratingCard: this.getSelectedRating()?.prefix || 'FUN',
            showMultiplayer: this.multiplayerTitleEnabled,
            showCustom: this.customTitleEnabled !== false
        };
    }
    ...
    getCustomLevelEntriesForSelectedRating() {
        const prefix = this.getSelectedRatingPrefix();
        return this.customLevelEntries
            .filter(entry => this.normaliseRatingPrefix(entry?.rating || entry?.pack || entry?.category) === prefix)
            .sort((a, b) => {
                const aNumber = Number(a.level_number ?? a.number ?? 0) || 0;
                const bNumber = Number(b.level_number ?? b.number ?? 0) || 0;
    ...
    getCustomLevelCount() {
        return Math.max(1, this.getCustomLevelEntriesForSelectedRating().length || 1);
    }

    getSelectedCustomLevelEntry() {
        if (this.levelSelectionMode !== 'custom') return null;
        return this.getCustomLevelEntriesForSelectedRating()[this.selectedLevelNumber - 1] || null;
    }

    getSelectedLevelId() {
        const rating = this.getSelectedRating();
        if (this.levelSelectionMode === 'custom') {
            const entry = this.getSelectedCustomLevelEntry();
            return entry?.id || `${rating.prefix}_${String(this.selectedLevelNumber).padStart(2, '0')}`;
        }
    ...
    changeSelectedLevel(deltaLevel, deltaRating) {
        if (deltaRating) {
            ...
            const count = this.levelSelectionMode === 'custom' ? this.getCustomLevelCount() : 30;
            this.selectedLevelNumber = ((this.selectedLevelNumber - 1 + count) % count) + 1;
        }

        if (deltaLevel) {
            const count = this.levelSelectionMode === 'custom' ? this.getCustomLevelCount() : 30;
            this.selectedLevelNumber = ((this.selectedLevelNumber - 1 + deltaLevel + count) % count) + 1;
        }
    ...
    async loadSelectedLevelInfo() {
        ...
        if (this.levelSelectionMode === 'custom' && !this.getCustomLevelEntriesForSelectedRating().length) {
            this.selectedLevelInfoId = levelId;
            this.selectedLevelInfo = {
                title: `NO CUSTOM ${rating.label}`,
                numLemmings: 0,
                percentNeeded: 0,
                releaseRate: 0,
                timeMinutes: 0,
        ...
        try {
            const customEntry = this.getSelectedCustomLevelEntry();
            const ini = await this.levelLoader.loadLevelInfo(levelId, { source: this.levelSelectionMode, entry: customEntry });
            if (requestId !== this.levelInfoRequestId) return;

            this.selectedLevelInfo = {
                title: ini.name || customEntry?.title || levelId,
    ...
    getTitleMenuCardRects() {
        ...
        const cardWidth = 48;
        const cardHeight = 56;
        const showCustom = this.customTitleEnabled !== false;
        const cardColumns = this.multiplayerTitleEnabled
            ? (showCustom
                ? [centerColumn - 15, centerColumn - 9, centerColumn - 3, centerColumn + 3, centerColumn + 9]
    ...
    handleTitleScreenInput() {
        const showCustom = this.customTitleEnabled !== false;
        const maxTitleSelectorIndex = this.multiplayerTitleEnabled
            ? (showCustom ? 4 : 3)
            : (showCustom ? 3 : 2);
        const pointer = this.input?.pointer;
        ...
        const selectPressed = this.input.wasJustPressed('select');
        ...
        if (!this.multiplayerTitleEnabled) {
            if (this.titleSelectorIndex === 0) {
                this.beginScreenTransition(() => this.startLevelSelection(this.levelSelectionMode));
        ...
        if (this.titleSelectorIndex === 0) {
            ...
            this.beginScreenTransition(() => this.enterCreditsScreen());
        } else if (this.titleSelectorIndex === 3) {
            this.cycleTitleDifficulty();
        } else if (showCustom) {
            this.toggleTitleLevelSource();
        }
    ...
    getNextLevelSelection() {
        ...
        return {
            ...
            levelNumber: (this.selectedLevelNumber % count) + 1,
            mode: this.levelSelectionMode
        };
    }
    ...
    enterLevelInfoScreen(ratingIndex = this.selectedRatingIndex, levelNumber = this.selectedLevelNumber) {
        ...
        const count = this.levelSelectionMode === 'custom' ? this.getCustomLevelCount() : 30;
        this.selectedLevelNumber = ((levelNumber - 1 + count) % count) + 1;
        this.audio?.stopMusic?.();
    ...
    async startSelectedLevel(mode) {
        ...
        if (this.levelSelectionMode === 'custom' && !this.getSelectedCustomLevelEntry()) {
            await this.loadSelectedLevelInfo();
            return;
        }
        ...
        try {
            await this.loadLevel(levelId, { showBriefing: false, source: this.levelSelectionMode, entry: this.getSelectedCustomLevelEntry() });
            if (mode === 'preview') {
                this.startPreview();
            } else {
    ...
    async loadLevel(levelName, options = {}) {
        ...
        this.level = await this.levelLoader.loadLevel(levelName, { source: options.source || this.levelSelectionMode, entry: options.entry || this.getSelectedCustomLevelEntry() });
        ...
        if (showBriefing) {
            ...
            this.selectedLevelInfo = {
                title: this.level.name || options.entry?.title || levelName,
    ...
    handleSkillSelectionInput() {
        if (!this.skillOrder.length) return false;
    ...
    isTouchPointerActive() {
        const pointer = this.input?.pointer;
        return !!pointer?.leftDown && (pointer.pointerType === 'touch' || pointer.pointerType === 'pen');
    }

    isDirectPointerControlActive() {
        return this.mouseControlsEnabled || this.isTouchPointerActive();
    }

    syncCursorToPointer(force = false) {
        const pointer = this.input?.pointer;
        if ((!force && !this.isDirectPointerControlActive()) || !pointer?.insideCanvas || !this.cursor || !this.renderer) {
            return false;
        }
    ...
    updateMouseModeCameraNavigation(isLogicTick = false) {
        if (!this.isDirectPointerControlActive() || !this.level || !this.renderer) return false;
        ...
        if (isLogicTick) {
            ...
            if (this.isTouchPointerActive()) {
                moved = this.updateTouchPointerEdgeScroll() || moved;
            }
        }

        const wheelSteps = this.input.consumeWheelSteps?.() || 0;
        if (wheelSteps !== 0 && this.mouseControlsEnabled) {
    ...
    updateTouchPointerEdgeScroll() {
        const pointer = this.input?.pointer;
        if (!pointer?.leftDown || !pointer.insideCanvas || !this.renderer || !this.level) return false;

        // Touch direct-control uses the same edge feel as the SMS cursor, but
        // only inside the playfield area. HUD taps remain HUD taps.
        const margin = this.POINTER_EDGE_SCROLL_MARGIN_PX;
        const viewportWidth = this.renderer.viewportWidth;
        const viewportHeight = this.renderer.viewportHeight;
        if (pointer.y >= viewportHeight) return false;

        let directionX = 0;
        let directionY = 0;

        if (pointer.x <= margin) directionX = -1;
        else if (pointer.x >= viewportWidth - margin) directionX = 1;

        if (pointer.y <= margin) directionY = -1;
        else if (pointer.y >= viewportHeight - margin) directionY = 1;
        ...
        return this.scrollCameraBy(directionX * speed, directionY * speed);
    }

    isCursorHorizontallyOverSkillHudStrip() {
        if (!this.cursor || !this.renderer) return false;

        const screenX = this.cursor.x - this.renderer.camera.x;
        const tileSize = this.renderer.tileSize || 8;
        const hudOffsetX = this.renderer.hudGroupOffsetX || 0;
        const column = Math.floor((screenX - hudOffsetX) / tileSize);

        // Skill HUD tiles occupy columns 12..27. When the cursor is moving
        // down over this strip, let it enter the HUD instead of forcing a
        // vertical camera scroll. Holding Button 1 keeps the old scroll path.
        return column >= 13 && column <= 28;
    }

    updateCursorForCurrentInputMode(isLogicTick = false) {
        if (this.isDirectPointerControlActive()) {
            this.updateMouseModeCameraNavigation(isLogicTick);
            this.syncCursorToPointer(true);
            return this.cursor?.hoveredLemming || null;
        }

        return this.cursor.update(this.input, this.renderer, this.level, this.lemmings, {
            allowCameraScroll: isLogicTick,
            suppressDownwardVerticalCameraScroll: this.isCursorHorizontallyOverSkillHudStrip()
        });
    }
    ...
    update() {
        ...
        if (this.state === 'levelInfo') {
            ...
            if (this.input.wasJustPressed('left')) {
                ...
                this.beginScreenTransition(() => this.changeSelectedLevel(0, 1));
            } else if (this.input.wasJustPressed('button1')) {
                this.beginScreenTransition(() => this.startSelectedLevel('preview'));
            } else if (this.input.wasJustPressed('select')) {
        ...
        if (this.state === 'preview') {
            ...
            this.handleSkillSelectionInput();
