...
class Lemming {
    ...
    findMatchingExit(level) {
        ...
        const lemmingFeetY = this.y + this.height;
        ...
        for (const exit of exits) {
            ...
            if (xDist < 1 &&
                lemmingFeetY >= exit.y - 4 &&
                lemmingFeetY <= exit.y + 12) {
                return exit;
            }
        }
...
findBlockingLemmingDuringWalkerStep(level, probeX, previousX, previousFeetY, nextFeetY) {
    ...
    // Miner owns its animation frame. SMS ix+7 is an 18-frame loop.
    // JS sprite frame stays zero-based while the action frame is 1-based.
    ...
    const getFarSideX = () => {
        ...
        // Facing right starts from the lemming's leftmost pixel.
        // Facing left starts from the lemming's rightmost pixel.
    ...
    const addMinerStampRects = (rects, originX, feetY) => {
        const columnHeights = Physics.minerStampColumnHeights || [13, 13, 13, 13, 13];

        for (let column = 0; column < columnHeights.length; column++) {
    ...
    const hasMinerFloorSupport = () => {
        const feetY = Math.floor(this.y + this.height - 1);
        const startX = Math.floor(this.x) - 2;

        // SMS breakthrough check scans exactly 4 pixels under the new body
        // position. If any pixel is solid, keep mining.
    ...
    if (actionFrame === (Physics.minerEraseFrame ?? 3)) {
        ...
        // Stamp 1 starts at the far-side pixel.
        // Stamp 2 is exactly 1px further in the mining direction.
        // The overlap gives the verified 6px-wide Miner breakthrough.
        ...
        // Empty terrain is not failure; SMS eraser can pass through air.
        if (removalResult?.blocked) {
            resetToWalking();
            return;
        }
    ...
    if (actionFrame === (Physics.minerMoveFrame ?? 4)) {
        ...
        // SMS does not turn the Miner directly into a Faller here.
        // It becomes Walker if the 4-pixel floor scan is all air.
    ...
    // action flow follows the ASM:
    //   - if 12 bricks have been placed, hand into the existing shrug/finish
    //     visual path;
    //   - only do Builder work when ix+7/action frame == $09;
    //   - before stamping, check blocker reversal, two 10px wall columns, and
    //     the hard Y=$10 top clamp;
    //   - probe/write the 3px bridge stamp at X, X+dir, X+2*dir and Y-1;
    //   - tolerate obstruction on the first brick only;
    //   - then move 2px forward and 1px up.
    // -------------------------------------------------------------------------
    ...
    runBuilderAsmFrame(level) {
        ...
        // Two exact ASM wall scans:
        //   1) X + direction
        //   2) current X
        // Each scans from Y-1 upward for 10 pixels and stops building when
        // 3 or more solid pixels are found.
        if (this.shouldBuilderWallBonk(level, currentFeetY)) {
        ...
        const obstructionCount = this.countBuilderAsmBrickObstructions(level, currentX, brickY, dir);

        // The ROM increments ix+12 before this check and explicitly skips the
        // obstruction stop when the new count is 1. That lets the first brick
        // embed/overlap the starting surface; later bricks stop if any of the
        // three stamp pixels are already occupied by real terrain.
        if (nextBuildCount !== 1 && obstructionCount > 0) {
            this.stopBuilding(true);
            return;
        }
	...
	startExploding() {
		...
		// Decided on first updateExploding(level) tick, because that has level access.
		this.explosionSinks = null;
		this.bomberBangTicks = 0;
	}
